...
dataProd_rtn_4 <- filter(dataProd1, !is.na(rtn_4) & rtn_4 >= 0 & rtn_4 <= 1 & cnt_subs > 20)
...
dataProd_rtn_4[categorical_vars] <- lapply(dataProd_rtn_4[categorical_vars], as.factor)

# Ensure that factor levels are consistent across both training and test datasets
# This step ensures that levels in test set match the levels in training set, 
# avoiding the issue where some levels are missing in the training set.
for (var in categorical_vars) {
  levels(dataProd_rtn_4[[var]]) <- union(levels(dataProd_rtn_4[[var]]), levels(dataProd_rtn_4[[var]]))  # Ensure the levels are consistent
}
...
product_levels <- table(dataProd_rtn_4$product)
infrequent_levels <- names(product_levels[product_levels < threshold])
dataProd_rtn_4$product <- as.factor(ifelse(dataProd_rtn_4$product %in% infrequent_levels, "Other", as.character(dataProd_rtn_4$product)))

# Check the number of levels for each categorical variable
for (var in categorical_vars) {
  cat(paste(var, ": ", length(levels(dataProd_rtn_4[[var]])), " levels\n"))
}

# Split the data into training and test sets (80% train, 20% test)
set.seed(42)  # Set seed for reproducibility
trainIndex <- createDataPartition(dataProd_rtn_4$rtn_4, p = 0.8, list = FALSE)
trainData_prod_rtn_4 <- dataProd_rtn_4[trainIndex,]
testData_prod_rtn_4 <- dataProd_rtn_4[-trainIndex,]
...
for (var in categorical_vars) {
  ...
  testData_prod_rtn_4[[var]] <- factor(testData_prod_rtn_4[[var]], levels = levels(trainData_prod_rtn_4[[var]]))
}

# Verify the factor levels to make sure there are no NA values due to unmatched levels
summary(trainData_prod_rtn_4[categorical_vars])
summary(testData_prod_rtn_4[categorical_vars])

# Ensure that the columns in testData_prod_rtn_4 match the columns in trainData_prod_rtn_4
testData_prod_rtn_4 <- testData_prod_rtn_4[, names(trainData_prod_rtn_4)]


# Random Forest Model (Predicting rtn_4)
rf_prodmodel_rtn_4 <- randomForest(rtn_4 ~ 
                                     d7_canc +
                                     d7_snz +
                                     #slope_rtn_4 +
                                     avg_age_centered +
                                     ...
                                     cat +
                                     site,
                                   data = trainData_prod_rtn_4, ntree = 100, na.action = na.exclude)

# Predictions for Random Forest Model (rtn_4)
rf_prodpreds_rtn_4 <- predict(rf_prodmodel_rtn_4, newdata = testData_prod_rtn_4)

# Calculate the R² equivalent for Random Forest Model (rtn_4)
sst_rtn_4 <- sum((na.omit(testData_prod_rtn_4$rtn_4) - mean(na.omit(testData_prod_rtn_4$rtn_4)))^2)

# Create a logical index where both rtn_4 and rf_prodpreds_rtn_4 are not NA
complete_idx <- complete.cases(testData_prod_rtn_4$rtn_4, rf_prodpreds_rtn_4)

# Use the index to subset both vectors
observed2 <- testData_prod_rtn_4$rtn_4[complete_idx]
predicted2 <- rf_prodpreds_rtn_4[complete_idx]

# Now compute SSE using the aligned vectors
sse_rtn_4 <- sum((observed2 - predicted2)^2)

r_squared_rf_rtn_4 <- ifelse(var(na.omit(testData_prod_rtn_4$rtn_4)) == 0 | var(na.omit(rf_prodpreds_rtn_4)) == 0, 
                             NA, 
...
rf_importance_rtn_4 <- importance(rf_prodmodel_rtn_4)
...
complete_idx2 <- complete.cases(testData_prod_rtn_4$rtn_4, rf_prodpreds_rtn_4)

# Subset the observed and predicted values using the same index
obs2 <- testData_prod_rtn_4$rtn_4[complete_idx2]
pred2 <- rf_prodpreds_rtn_4[complete_idx2]
...
testData_prod_rtn_4 <- testData_prod_rtn_4 %>%
  mutate(predicted_rtn_4_rf_model = rf_prodpreds_rtn_4)
...
testData_prod_rtn_4_clean <- testData_prod_rtn_4 %>%
  filter(!is.na(rtn_4) & !is.na(predicted_rtn_4_rf_model))

ggplot(testData_prod_rtn_4_clean, aes(x = rtn_4, y = predicted_rtn_4_rf_model)) +
  geom_point(color = "green") +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(title = "Act vs Pred: RF Prod Model v1.23 (rtn_4)",
       x = "Actual Retention",
...
testData_prod_rtn_4[categorical_vars] <- lapply(testData_prod_rtn_4[categorical_vars], as.factor)

# Ensure that factor levels are consistent across both datasets
for (var in categorical_vars) {
  testData_prod_rtn_4[[var]] <- factor(testData_prod_rtn_4[[var]], levels = levels(dataProd1[[var]]))
}

# Now perform the left join with correct column names
dataProd1rf <- left_join(dataProd1, testData_prod_rtn_4 %>%
                           select(create_mth, site, prod_grp, product, supply, epi, prop, predicted_rtn_4_rf_model),
