...
dataCat_rtn_19 <- filter(dataCat1, !is.na(rtn_19) & rtn_19 >= 0 & rtn_19 <= 1)

# Split the data into training and test sets (80% train, 20% test)
set.seed(42)  # Set seed for reproducibility
trainIndex <- createDataPartition(dataCat_rtn_19$rtn_19, p = 0.8, list = FALSE)
trainData_cat_rtn_19 <- dataCat_rtn_19[trainIndex,]
testData_cat_rtn_19 <- dataCat_rtn_19[-trainIndex,]

# Ensure 'prod_grp' factor levels match between training and test data
testData_cat_rtn_19$cat_grp <- factor(testData_cat_rtn_19$cat_grp, levels = levels(trainData_cat_rtn_19$cat_grp))

str(trainData_cat_rtn_19)
str(testData_cat_rtn_19)

# Random Forest Model (Predicting rtn_19)
rf_catmodel_rtn_19 <- randomForest(rtn_19 ~ 
                                     d7_canc +
                                     d7_snz +
                                     rtn_4 +
                                     slope_rtn_4 +
                                     ...
                                     prop +
                                     site,
                                   data = trainData_cat_rtn_19, ntree = 100, na.action = na.exclude)

# Predictions for Random Forest Model (rtn_19)
rf_catpreds_rtn_19 <- predict(rf_catmodel_rtn_19, newdata = testData_cat_rtn_19)

# Calculate the R² equivalent for Random Forest Model (rtn_19)
sst_rtn_19 <- sum((na.omit(testData_cat_rtn_19$rtn_19) - mean(na.omit(testData_cat_rtn_19$rtn_19)))^2)

# Create a logical index where both rtn_19 and rf_catpreds_rtn_19 are not NA
complete_idx <- complete.cases(testData_cat_rtn_19$rtn_19, rf_catpreds_rtn_19)

# Use the index to subset both vectors
observed2 <- testData_cat_rtn_19$rtn_19[complete_idx]
predicted2 <- rf_catpreds_rtn_19[complete_idx]

# Now compute SSE using the aligned vectors
sse_rtn_19 <- sum((observed2 - predicted2)^2)

r_squared_rf_rtn_19 <- ifelse(var(na.omit(testData_cat_rtn_19$rtn_19)) == 0 | var(na.omit(rf_catpreds_rtn_19)) == 0, 
                              NA, 
...
rf_importance_rtn_19 <- importance(rf_catmodel_rtn_19)
...
complete_idx2 <- complete.cases(testData_cat_rtn_19$rtn_19, rf_catpreds_rtn_19)

# Subset the observed and predicted values using the same index
obs2 <- testData_cat_rtn_19$rtn_19[complete_idx2]
pred2 <- rf_catpreds_rtn_19[complete_idx2]
...
testData_cat_rtn_19 <- testData_cat_rtn_19 %>%
  mutate(predicted_rtn_19_rf_model = rf_catpreds_rtn_19)
...
# Remove rows with NA in 'rtn_19' or 'predicted_rtn_19_rf_model'
testData_cat_rtn_19_clean <- testData_cat_rtn_19 %>%
  filter(!is.na(rtn_19) & !is.na(predicted_rtn_19_rf_model))

# Plot for Random Forest Model with RMSE and R² annotations
ggplot(testData_cat_rtn_19_clean, aes(x = rtn_19, y = predicted_rtn_19_rf_model)) +
  geom_point(color = "green") +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(title = "Act vs Pred: RF Cat Model v1.23 (rtn_19)",
       x = "Actual Retention",
...
dataCat1rf <- left_join(dataCat1, testData_cat_rtn_19 %>%
                          select(create_mth, site, cat, supply, epi, prop, predicted_rtn_19_rf_model), 
