...
# Define a function to process each metric and calculate the weighted average per cat_grp
process_metric <- function(metric_name, dataCat) {
  # Filter the data for the given metric where it's not NA and cnt_subs > 10
  data_metric <- dataCat %>%
    filter(!is.na(!!sym(metric_name)) & cnt_subs > 10) %>%
    arrange(cat_grp, desc(create_mth)) %>%
    group_by(cat_grp) %>%
    mutate(row_num = row_number()) %>%
    ungroup()
  
  # Get the top 3 rows per cat_grp for the given metric
  data_metric_top3 <- data_metric %>%
    filter(row_num <= 3) %>%
    select(cat_grp, create_mth, cnt_subs, !!sym(metric_name), row_num)
  
  # Calculate the weighted average per cat_grp for the given metric
...
for (metric in metrics) {
  data_processed <- process_metric(metric, dataCat)
  
  # Add the data to the final_data_cat table
  if (is.null(final_data_cat)) {
    final_data_cat <- data_processed
  } else {
    final_data_cat <- bind_rows(final_data_cat, data_processed)
  }
}

# Add the new column where cnt_subs < 50 gets 1, else 0
final_data_cat <- final_data_cat %>%
  mutate(cnt_subs_check = if_else(cnt_subs < 50, 1, 0))

# View the final combined data with weighted averages per cat_grp
print(final_data_cat)

# ---- SECTION 2 

# Calculate the max of each specified column for each cat_grp
# Calculate the max of each specified column for each cat_grp and handle missing values
...
final_summary_cat <- final_data_cat %>%
  group_by(cat_grp) %>%
  summarize(
...
print(final_summary_cat)

# Write the final summary table to a CSV file
write.csv(final_summary_cat, "final_summary_cat.csv", row.names = FALSE)
