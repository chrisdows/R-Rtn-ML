...
dataProd_rtn_13 <- filter(dataProd1, !is.na(rtn_13) & rtn_13 >= 0 & rtn_13 <= 1 & cnt_subs > 20)
...
dataProd_rtn_13[categorical_vars] <- lapply(dataProd_rtn_13[categorical_vars], as.factor)

# Ensure that factor levels are consistent across both training and test datasets
for (var in categorical_vars) {
  levels(dataProd_rtn_13[[var]]) <- union(levels(dataProd_rtn_13[[var]]), levels(dataProd_rtn_13[[var]]))  # Ensure the levels are consistent
}
...
product_levels <- table(dataProd_rtn_13$product)
infrequent_levels <- names(product_levels[product_levels < threshold])
dataProd_rtn_13$product <- as.factor(ifelse(dataProd_rtn_13$product %in% infrequent_levels, "Other", as.character(dataProd_rtn_13$product)))

# Check the number of levels for each categorical variable
for (var in categorical_vars) {
  cat(paste(var, ": ", length(levels(dataProd_rtn_13[[var]])), " levels\n"))
}

# Split the data into training and test sets (80% train, 20% test)
set.seed(42)  # Set seed for reproducibility
trainIndex <- createDataPartition(dataProd_rtn_13$rtn_13, p = 0.8, list = FALSE)
trainData_prod_rtn_13 <- dataProd_rtn_13[trainIndex,]
testData_prod_rtn_13 <- dataProd_rtn_13[-trainIndex,]
...
for (var in categorical_vars) {
  ...
  testData_prod_rtn_13[[var]] <- factor(testData_prod_rtn_13[[var]], levels = levels(trainData_prod_rtn_13[[var]]))
}

# Verify the factor levels to make sure there are no NA values due to unmatched levels
summary(trainData_prod_rtn_13[categorical_vars])
summary(testData_prod_rtn_13[categorical_vars])

# Ensure that the columns in testData_prod_rtn_13 match the columns in trainData_prod_rtn_13
testData_prod_rtn_13 <- testData_prod_rtn_13[, names(trainData_prod_rtn_13)]


# Random Forest Model (Predicting rtn_13)
rf_prodmodel_rtn_13 <- randomForest(rtn_13 ~ 
                                      d7_canc +
                                      d7_snz +
                                      rtn_4 +
                                      slope_rtn_4 +
                                      ...
                                      cat + 
                                      site,
                                    data = trainData_prod_rtn_13, ntree = 100, na.action = na.exclude)

# Predictions for Random Forest Model (rtn_13)
rf_prodpreds_rtn_13 <- predict(rf_prodmodel_rtn_13, newdata = testData_prod_rtn_13)

# Calculate the R² equivalent for Random Forest Model (rtn_13)
sst_rtn_13 <- sum((na.omit(testData_prod_rtn_13$rtn_13) - mean(na.omit(testData_prod_rtn_13$rtn_13)))^2)

# Create a logical index where both rtn_13 and rf_prodpreds_rtn_13 are not NA
complete_idx <- complete.cases(testData_prod_rtn_13$rtn_13, rf_prodpreds_rtn_13)

# Use the index to subset both vectors
observed2 <- testData_prod_rtn_13$rtn_13[complete_idx]
predicted2 <- rf_prodpreds_rtn_13[complete_idx]

# Now compute SSE using the aligned vectors
sse_rtn_13 <- sum((observed2 - predicted2)^2)

r_squared_rf_rtn_13 <- ifelse(var(na.omit(testData_prod_rtn_13$rtn_13)) == 0 | var(na.omit(rf_prodpreds_rtn_13)) == 0, 
                              NA, 
...
rf_importance_rtn_13 <- importance(rf_prodmodel_rtn_13)
...
complete_idx2 <- complete.cases(testData_prod_rtn_13$rtn_13, rf_prodpreds_rtn_13)

# Subset the observed and predicted values using the same index
obs2 <- testData_prod_rtn_13$rtn_13[complete_idx2]
pred2 <- rf_prodpreds_rtn_13[complete_idx2]
...
testData_prod_rtn_13 <- testData_prod_rtn_13 %>% 
  mutate(predicted_rtn_13_rf_model = rf_prodpreds_rtn_13)
...
testData_prod_rtn_13_clean <- testData_prod_rtn_13 %>% 
  filter(!is.na(rtn_13) & !is.na(predicted_rtn_13_rf_model))

ggplot(testData_prod_rtn_13_clean, aes(x = rtn_13, y = predicted_rtn_13_rf_model)) + 
  geom_point(color = "green") + 
  geom_abline(slope = 1, intercept = 0, color = "red") + 
  labs(title = "Act vs Pred: RF Prod Model v1.23 (rtn_13)",
       x = "Actual Retention",
...
testData_prod_rtn_13[categorical_vars] <- lapply(testData_prod_rtn_13[categorical_vars], as.factor)

# Ensure that factor levels are consistent across both datasets
for (var in categorical_vars) {
  testData_prod_rtn_13[[var]] <- factor(testData_prod_rtn_13[[var]], levels = levels(dataProd1[[var]]))
}

# Perform the left join with correct column names
dataProd1rf <- left_join(dataProd1, testData_prod_rtn_13 %>% 
                           select(create_mth, site, prod_grp, product, supply, epi, prop, predicted_rtn_13_rf_model),
